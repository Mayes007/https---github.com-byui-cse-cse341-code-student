...
const next_1 = require("@alife/next");
class App extends React.Component {
    ...
    columns = [
        ...
        },
        {
            ...
            cell: (value, index, record) => {
                if (value) {
                    return value.resource_key ? (<p>{value.resource_key}</p>) : (<p>{`文案key查询失败:${value.errMsg}`}</p>);
                }
                else {
                    return "--";
                }
        ...
        // },
        {
            ...
            dataIndex: "language",
            cell: (value, index, record) => {
                return (<next_1.Select defaultValue={value} value={value} onChange={(val) => this.onLanguageChange(val, index, record)}>
            <next_1.Select.Option value="zh_CN">中文</next_1.Select.Option>
            <next_1.Select.Option value="en_US">英文</next_1.Select.Option>
          </next_1.Select>);
            },
        },
    ];
    ...
    onLanguageChange = (val, index, record) => {
        let tableDataSource = JSON.parse(JSON.stringify(this.state.tableDataSource));
        record.language = val;
    ...
    onMcmsChecked = (checked, index, record) => {
        ...
        if (!checked) {
            if (record.keyMcmsError)
                record.keyMcmsError = false;
            if (!record.mdsKeyAutoGenerate)
                record.keyAutoError = true;
        }
        tableDataSource[index] = record;
    ...
    onConfirm = () => {
        ...
        if (!selectedRowKeys.length) {
            next_1.Message.warning("没有可修复的文案");
            return;
        }
        let tableDataSource = JSON.parse(JSON.stringify(this.state.tableDataSource));
        // 选中的才替换
    ...
    render() {
        ...
        <next_1.Table className="medusa-review-table" dataSource={tableDataSource} cellProps={this.cellProps} isZebra={true} primaryKey="key" rowSelection={this.state.rowSelection}>
          {this.columns.map((item) => {
                return (<next_1.Table.Column title={item.title} dataIndex={item.dataIndex} cell={item.cell}></next_1.Table.Column>);
            })}
        </next_1.Table>
        <div className="app-btn-wrap">
          <next_1.Button onClick={this.onConfirm} type="primary" className="app-btn-wrap-confirm">
            确认
          </next_1.Button>
          <next_1.Button onClick={this.onCancel}>取消</next_1.Button>
        </div>
      </div>);
    }
}
