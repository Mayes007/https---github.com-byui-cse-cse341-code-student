...
export default class BuildAndRunAvaSingleTestCommand implements ICommandHandler {
	constructor(private readonly _taskRunner: ITaskRunner) { }

	public get options(): ICommandOptions {
		return {
			name: "cs-test-runner.build-run-ava"
		};
	}
	...
	public handle(payload: ICommandPayload) {
		...
		this._taskRunner.run(`cd ${projectDir} && pnpm build:test && ./node_modules/.bin/ava ${testDir} -m "${testCase}"`);
	}
}
